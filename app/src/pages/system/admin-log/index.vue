...
export default class AdminLog extends Vue {
    ...
    return [
      ...
      },
      {
        ...
        clickFun: (obj, index?: number) => {
          ...
          api
            .logAdminInfo({ admin_id: id, item_id })
            .then((res: any) => {
              const { code, data } = res
              if (code === 0) {
  ...
  /** 方法 */
  requestTable() {
    const { hall_id, operator_name, target_name, log_ip, module_id, menu_id, item_id, status, date } = this.filterForm
  ...
  requestSelect() {
    // 1为厅主日志，2总后台日志
    api.logSelect({ type: 1 }).then((res: any) => {
      const { code, data } = res
      if (code === 0) {
  ...
  requestSelectList() {
    // 1厅主2总后台3会员4代理
    api.logSelectList({ type: 1 }).then((res: any) => {
      const { code, data } = res
      if (code === 0) {
